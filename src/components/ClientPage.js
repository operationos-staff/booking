...
export default function ClientPage({ data }) {
  ...
  const isAvia = data._kind === 'avia'
  const isFishing = data._kind === 'fishing'
  const isCustom = data._kind === 'custom'
  const isVacation = data._kind === 'vacation'
  const routeIcon = isVacation ? '📅' : isCustom ? '🧩' : isFishing ? '🎣' : isAvia ? '✈️' : isIndividual ? '👤' : isSights ? '🏛️' : isLand ? '🏔️' : '🚤'
  const routeBaseLabel = isVacation ? 'Программа отдыха по дням'
                       : isCustom ? 'Индивидуальная программа'
                       : isFishing ? 'Рыбалка по программе'
                       : isAvia ? 'Авиатур по программе'
                       : isIndividual ? 'Индивидуальная экскурсия по программе'
