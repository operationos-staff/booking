  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', paddingTop: '80px', paddingLeft: '16px', paddingRight: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ color: 'var(--txl)', fontSize: '14px' }}>Загрузка статистики...</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '80px', paddingBottom: '100px', paddingLeft: '16px', paddingRight: '16px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: 'var(--txl)', fontSize: '13px' }}>Активность системы бронирования</p>
      </div>
      ...
      {/* Bar chart */}
      {dates.length > 0 && (
        <div style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: '12px', padding: '16px 16px 24px', marginBottom: '20px' }}>
            ...
            {dates.map(d => {
              ...
              return (
                <div key={d} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '8px', color: 'var(--txm)', whiteSpace: 'nowrap' }}>{d.slice(5)}</div>
                </div>
              )
            })}
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        {/* Recent calculations */}
        <div style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: '12px', padding: '16px' }}>
          ...
          {stats.recentCalcs.length === 0 ? (
            <div style={{ color: 'var(--txm)', fontSize: '12px' }}>Нет данных</div>
          ) : (
            stats.recentCalcs.slice(0, 10).map((c, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: i < Math.min(9, stats.recentCalcs.length - 1) ? '1px solid var(--dv)' : 'none', fontSize: '12px' }}>
                <span style={{ color: 'var(--txt)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, marginRight: '8px' }}>
                  {c.client_name || '—'}
                </span>
                <span style={{ color: 'var(--txm)', flexShrink: 0 }}>
        ...
        <div style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: '12px', padding: '16px' }}>
          ...
          {stats.recentLogins.length === 0 ? (
            ...
            stats.recentLogins.map((l, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: i < stats.recentLogins.length - 1 ? '1px solid var(--dv)' : 'none', fontSize: '12px' }}>
                <span style={{ color: 'var(--txt)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, marginRight: '8px' }}>
                  {l.user_email}
                </span>
                <span style={{ color: 'var(--txm)', flexShrink: 0 }}>
...
function KpiCard({ icon, label, value, color }) {
  return (
    <div style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: '12px', padding: '16px', textAlign: 'center' }}>
      <div style={{ fontSize: '22px', marginBottom: '8px' }}>{icon}</div>
      <div style={{ fontSize: '28px', fontWeight: 900, color, lineHeight: 1, marginBottom: '6px' }}>{value}</div>
      <div style={{ fontSize: '11px', color: 'var(--txl)', fontWeight: 600 }}>{label}</div>
    </div>
  )
}
